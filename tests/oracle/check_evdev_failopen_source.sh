...
grep -q 'LinuxUinputRawKeyEvent' "$EV"
grep -q 'SYN_DROPPED' "$EV"
grep -q 'ReconcileLocalDevice' "$EV"
grep -q 'StartLocalGrabWatchdog' "$EV"
grep -q 'ServiceLocalGrabWatchdogs' "$EV"
grep -q 'ReleaseLocalGrabsForReplayFailure' "$EV"
grep -q 'bool WriteUinputEvent' "$UI"
grep -q 'return WriteUinputEvent(EV_KEY' "$UI"
grep -q 'LinuxUinputRawKeyEvent' "$UI"
