...
| **合计 (X11/headless,各 expect 文件行数之和 + assert_display_content 自由格式 4 项;由 verify_report_numbers.sh 机器校验,与 run_check.sh 实际 PASS 一致)** | | **1152** |
...
普通构建: tests/run_tests.sh        PASS=27 FAIL=0
          tests/doccheck/run_check.sh --xvfb PASS=1152 FAIL=0
