#!/bin/bash
# Debug v2: ImageSearch on sway's XWayland via wlr-screencopy fallback.
set -x
D=/var/tmp/xwimg2
...
255 0 0 255 0 0
EOF
# Start the red-filled test window, then search for the 2x2 red image.
...
r := ImageSearch(&px, &py, 0, 0, 1279, 719, "/tmp/ahk_dc_img/red2.ppm")
FileAppend("ImageSearch rc=" r " x=" px " y=" py "`n", "/tmp/xw_img2.txt")
try {
    c := PixelGetColor(60, 80)
    FileAppend("PixelGetColor(60,80)=" c "`n", "/tmp/xw_img2.txt")
} catch as e {
    FileAppend("PixelGetColor err: " Type(e) " " e.Message "`n", "/tmp/xw_img2.txt")
}
ExitApp(0)
EOF
rm -f /tmp/xw_img2.txt
timeout 30 /mnt/f/AI/Codex/Autohotkey_Linux/build-core/source/linux/core/ahk_core $D/img_min.ahk > $D/run.log 2>&1
echo "runner rc=$? (124 = hang)"
cat /tmp/xw_img2.txt 2>/dev/null || echo "no img txt"
tail -3 $D/run.log
