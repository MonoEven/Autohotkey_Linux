...
if [ "$XVFB" = 1 ]; then
  if ! command -v Xvfb >/dev/null 2>&1 || ! command -v gcc >/dev/null 2>&1; then
    ...
    # setxkbmap (layout switch test) is part of the standard X11 utilities.
    if ! command -v setxkbmap >/dev/null 2>&1; then
      echo "NOTE: setxkbmap not found; assert_layout will fail loudly" >&2
    fi
...
for ahk in assert_*.ahk; do
  ...
  if [ "$base" = "assert_hotkey_lr" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_hotkey_lr (run with --xvfb: xkeycap foreground client)"
    continue
  fi
  if [ "$base" = "assert_hotstring" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_hotstring (run with --xvfb: xkeycap foreground client)"
    continue
  fi
  if [ "$base" = "assert_inputhook" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_inputhook (run with --xvfb: xkeycap foreground client)"
    continue
  fi
  if [ "$base" = "assert_unicode_lease" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_unicode_lease (run with --xvfb: xkeycap foreground client)"
    continue
  fi
  ...
  if [ "$base" = "assert_repeat" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_repeat (run with --xvfb: auto-repeat + xkeycap)"
    continue
  fi
  if [ "$base" = "assert_layout" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_layout (run with --xvfb: layout switch + xkeycap)"
    continue
  fi
  ...
  if [ "$base" = "assert_clipboard_all" ] && [ "$XVFB" != 1 ]; then
  ...
  if [ "$base" = "assert_clipboard_change" ] && [ "$XVFB" != 1 ]; then
    echo "SKIP: assert_clipboard_change (run with --xvfb: OnClipboardChange + xclip)"
    continue
  fi
  ...
  case "$base" in
    ...
    assert_hotkey_lr) out_src="/tmp/ahk_dc_hotkeylr_out.txt" ;;
    assert_hotstring) out_src="/tmp/ahk_dc_hotstring_out.txt" ;;
    assert_inputhook) out_src="/tmp/ahk_dc_inputhook_out.txt" ;;
    assert_unicode_lease) out_src="/tmp/ahk_dc_lease_out.txt" ;;
