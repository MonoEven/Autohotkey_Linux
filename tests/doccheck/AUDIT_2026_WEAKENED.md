...
- [主要→大部分已修复(round-29,check0818 审计)] **热键后端**:
  ...
  已完成(round-30/31,见 `audits/check0818.md`):**鼠标热键**(XGrabButton,
  round-30)、**左右修饰键区分与通配修饰键 `*`**(round-31:XI2 raw 事件
  观察器跟踪修饰键左右侧——抓取只匹配掩码,事件处理按物理键码判侧,
  错侧透传;通配把抓取展开到主修饰组合全集)与**哈希索引唯一决议**。
  剩余:扫描码、`A & B` 前缀——需按键缓冲状态机/统一事件流(计划中,
  不再静默注册假热键:能力校验在注册时拒绝并向脚本报错)。
