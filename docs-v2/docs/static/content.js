...
function ctor_structure()
{
  ...
  self.modify = function() { // Modify elements added via build.
    ...
    // Linux port: English manual, v2 only.
var lang = "en", ver = "v2";
    // language links. Keys are based on ISO 639-1 language name standard:
    // Linux port fork: only English (this site) and Chinese (this site's
// maintained zh page) are offered; links never leave this fork.
var link = { 'v2': { 'en': scriptDir.replace(/\/static$/, '') + '/',
                         'zh': scriptDir.replace(/\/static$/, '') + '/zh.htm' } }
    ...
    self.modifyTools = function(relPath, equivPath) {
      ...
      $langList.find('li').each( function() {
        var a = $(this).find('a');
        var lang = isIE8 ? a.text() : a.attr('data-content');
        if (lang == 'zh')
          a.attr('href', scriptDir.replace(/\/static$/, '') + '/zh.htm'); // Maintained Chinese page (fixed).
        else if (lang == 'en')
          a.attr('href', relPath); // Stay on this site, same page.
        else
          $(this).hide(); // Hide unsupported language buttons.
      });
